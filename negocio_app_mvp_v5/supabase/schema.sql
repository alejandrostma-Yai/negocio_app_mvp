...
begin
  ...
  from public.sales
  where user_id=v_user and sale_date=p_business_date and status='pendiente';
  ...
  from public.sales
  where user_id=v_user and sale_date=p_business_date and status='completada';

  v_cost_per_unit := v_settings.mp1 + v_settings.mp2 + v_settings.mp3;
  v_replacement := v_count * v_cost_per_unit;

  if v_gross < v_replacement then
    raise exception 'El monto bruto no cubre el capital de reposición calculado';
  end if;

  v_house := least(v_settings.daily_house_amount, greatest(v_gross - v_replacement, 0));
  ...
  on conflict(user_id,business_date) do update set
    ...
    replacement_capital=excluded.replacement_capital,
    house_allocation=excluded.house_allocation,
    goal_allocation=excluded.goal_allocation,
    daily_unit_goal=excluded.daily_unit_goal,
    mp1_snapshot=excluded.mp1_snapshot,
    mp2_snapshot=excluded.mp2_snapshot,
    mp3_snapshot=excluded.mp3_snapshot,
    house_amount_snapshot=excluded.house_amount_snapshot,
    closed_at=now();
  ...
  insert into public.transaction_history(user_id,type,amount,direction,description)
  values
    (v_user,'cierre_capital',v_replacement,'ingreso','Reposición de capital del cierre'),
    (v_user,'cierre_casa',v_house,'ingreso','Aporte del día al fondo de casa'),
    (v_user,'cierre_meta',v_goal,'ingreso','Aporte del día a la meta');
