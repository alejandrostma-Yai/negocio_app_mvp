...
export default function VistaRapida(){
  ...
  return <AuthGuard><main className={`quick-view-shell${open ? " has-drawer" : ""}`}>
    ...
    {open!==null&&<section className={`quick-drawer quick-drawer-${open}`}>
      ...
      {open==="sales"&&(pendingSales.length===0?<p className="muted">No hay citas pendientes para hoy.</p>:pendingSales.slice(0,3).map(v=><Link href={`/agenda?date=${dateKey()}`} className="quick-list-row" key={v.id}><div><strong>{v.client_name}</strong><span>{clock(v.sale_time)} · RD${money(v.price)}</span></div><b>Pendiente</b></Link>))}
      {open==="phones"&&(data.phones.length===0?<p className="muted">No hay números pendientes de llamar para hoy.</p>:data.phones.slice(0,3).map(v=><Link href={`/agenda?date=${dateKey()}`} className="quick-list-row" key={v.id}><div><strong>{v.client_name}</strong><span>{v.phone} · {clock(v.sale_time)}</span></div><b>{v.contact_status==="llamo"?"Llamó":"Pendiente"}</b></Link>))}
      {open==="notes"&&(data.notes.length===0?<p className="muted">No hay notas del Bloc modificadas hoy.</p>:data.notes.slice(0,3).map(n=><Link href="/link/bloc" className="quick-note-row" key={n.id}><span>{n.note.trim()||"Nota sin texto"}</span><small>{new Date(n.updated_at).toLocaleTimeString("es-DO",{hour:"2-digit",minute:"2-digit",timeZone:"America/Santo_Domingo"})}</small></Link>))}
      {open==="sales"&&pendingSales.length>3&&<Link className="quick-more" href={`/agenda?date=${dateKey()}`}>Ver todas las citas</Link>}
      {open==="phones"&&data.phones.length>3&&<Link className="quick-more" href={`/agenda?date=${dateKey()}`}>Ver todos los pendientes</Link>}
      {open==="notes"&&data.notes.length>3&&<Link className="quick-more" href="/link/bloc">Ver todas las notas</Link>}
    </section>}
