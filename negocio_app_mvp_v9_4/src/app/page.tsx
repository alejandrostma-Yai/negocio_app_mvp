...
export default function Home() {
  ...
  const cargar = useCallback(async () => {
    ...
    const week = weekInfo();
    const [walletRes, settingsRes, salesRes, completedRes, createdRes, pendingRes, historyRes, closureRes, weekCompletedRes, weekClosureRes] = await Promise.all([
      supabase.from("wallets").select("working_capital,house_fund,goal_balance").eq("user_id", user.id).single(),
      supabase.from("settings").select("goal_amount,daily_unit_goal,mp1,mp2,mp3,daily_house_amount,require_no_pending_to_close,home_note,close_mode").eq("user_id", user.id).single(),
      ...
      supabase.from("sales").select("id,price,completed_at").eq("user_id", user.id).eq("status", "completada").gte("completed_at", week.start).lt("completed_at", week.end),
      supabase.from("weekly_closures").select("id").eq("user_id", user.id).eq("week_start", week.date).maybeSingle()
    ]);

    const firstError = walletRes.error || settingsRes.error || salesRes.error || completedRes.error || createdRes.error || pendingRes.error || historyRes.error || closureRes.error || weekCompletedRes.error || weekClosureRes.error;
    ...
    const weekRows = weekCompletedRes.data ?? [];
    const weekGross = weekRows.reduce((sum, sale) => sum + Number(sale.price), 0);
    const workedDays = new Set(weekRows.map(sale => new Date(String(sale.completed_at)).toLocaleDateString("en-CA", { timeZone: "America/Santo_Domingo" }))).size;
    ...
    setData({
      ...
      homeNote: String(settingsRes.data?.home_note ?? ""),
      closeMode: settingsRes.data?.close_mode === "weekly" ? "weekly" : "daily",
      weekGross,
      weekCompleted: weekRows.length,
      weekWorkedDays: workedDays,
      weekClosed: Boolean(weekClosureRes.data)
    });
    setCargando(false);
  }, []);
  ...
  const cierreInvalidoPorPendientes = data.closeMode === "daily" && data.requireNoPendingToClose && agendaPendiente.length > 0;
  const weekCapitalCierre = data.weekCompleted * capitalPorVenta;
  const weekDisponibleDespuesCapital = Math.max(data.weekGross - weekCapitalCierre, 0);
  const weekCasaCierre = Math.min(data.dailyHouseAmount * data.weekWorkedDays, weekDisponibleDespuesCapital);
  const weekMetaCierre = Math.max(data.weekGross - weekCapitalCierre - weekCasaCierre, 0);
  const weekCierreInvalidoPorCapital = data.weekGross < weekCapitalCierre;
  const cierreYaHecho = data.closeMode === "weekly" ? data.weekClosed : data.closedToday;
  const cierreCount = data.closeMode === "weekly" ? data.weekCompleted : data.completedToday;
  const cierreCapital = data.closeMode === "weekly" ? weekCapitalCierre : capitalCierre;
  const cierreCasa = data.closeMode === "weekly" ? weekCasaCierre : casaCierre;
  const cierreMeta = data.closeMode === "weekly" ? weekMetaCierre : metaCierre;
  const cierreCapitalInvalido = data.closeMode === "weekly" ? weekCierreInvalidoPorCapital : cierreInvalidoPorCapital;

  async function ejecutarCierre() {
    if (cierreYaHecho || cerrando) return;
    setCerrando(true);
    setMensaje("");
    const supabase = createClient();
    const week = weekInfo();
    const { error } = data.closeMode === "weekly"
      ? await supabase.rpc("close_business_week", { p_week_start: week.date })
      : await supabase.rpc("close_business_day", { p_business_date: todayLocal() });
    if (error) { setMensaje(error.message); setCerrando(false); return; }
    setMensaje(data.closeMode === "weekly" ? "Semana cerrada correctamente." : "Día cerrado correctamente.");
      ...
      <main className="shell">
        ...
        <section className="home-greeting" aria-label="Saludo del usuario">
          <div className="home-greeting-copy">
            <div className="home-title-row"><div className="home-greeting-title">Hola, {nombreUsuario}</div><a className="home-bloc-button" href="/link/bloc">Bloc</a></div>
            <div className="home-greeting-subtitle">Bienvenido de nuevo</div>
          </div>
          <div className="daily-goal-mini" aria-label={`Meta diaria ${progresoUnidades.toFixed(0)}%`}>
            <div className="daily-goal-mini-top">
              <span>Meta diaria</span>
              <strong>{progresoUnidades.toFixed(0)}%</strong>
            </div>
            <div className="daily-goal-mini-track"><div style={{width: `${progresoUnidades}%`}} /></div>
          </div>
        </section>
        ...
        <CloseCountdown mode={data.closeMode} closed={cierreYaHecho} />

        <div className="close-day-button-wrap">
          {cierreYaHecho ? (
            <button className="btn close-day-button" type="button" disabled>{data.closeMode === "weekly" ? "Semana cerrada ✓" : "Día cerrado ✓"}</button>
          ) : (
            <button
              className="btn close-day-button"
              type="button"
              onClick={() => setMostrarCierre(true)}
              disabled={cargando || cerrando || cierreCount === 0}
            >
              {data.closeMode === "weekly" ? "Cerrar semana" : "Cerrar día"}
            </button>
          )}
        </div>


        {mostrarCierre && !cierreYaHecho && (
          <div className="modal-backdrop" role="presentation" onMouseDown={() => !cerrando && setMostrarCierre(false)}>
            <section className="close-preview-modal" role="dialog" aria-modal="true" aria-labelledby="close-preview-title" onMouseDown={e => e.stopPropagation()}>
              ...
              <h2 id="close-preview-title">{data.closeMode === "weekly" ? "Cierre de la semana" : "Cierre del día"}</h2>
              <div className="close-preview-grid">
                <div><span>Capital de trabajo</span><strong>RD${money(cierreCapital)}</strong></div>
                <div><span>Casa</span><strong>RD${money(cierreCasa)}</strong></div>
                <div className="close-preview-goal"><span>Meta</span><strong>RD${money(cierreMeta)}</strong></div>
              </div>
              <div className="close-preview-note">{cierreCount} orden{cierreCount === 1 ? "" : "es"} cobrada{cierreCount === 1 ? "" : "s"} {data.closeMode === "weekly" ? "esta semana" : "hoy"}. Solo se calcula lo cobrado/completado.</div>
              ...
              {cierreCapitalInvalido && <div className="close-preview-warning">El monto bruto no cubre el costo configurado de materias primas.</div>}
              <div className="close-preview-actions">
                ...
                <button className="btn" type="button" onClick={() => void ejecutarCierre()} disabled={cerrando || cierreInvalidoPorPendientes || cierreCapitalInvalido}>
        ...
        <div className="card home-note-card">
          {data.homeNote.trim() ? data.homeNote : <span className="muted">Sin nota configurada.</span>}
        </div>
