...
export default function ConfiguracionPage() {
  ...
  const [tipoCierre, setTipoCierre] = useState<CloseMode>("daily");
  ...
  useEffect(() => {
    async function cargar() {
      ...
      if (data) {
        ...
        setTipoCierre(data.close_mode === "weekly" ? "weekly" : "daily");
      }
      setCargando(false);
    }
    void cargar();
      ...
      <main className="shell">
        ...
        <form className="settings-section" onSubmit={guardar}>
          <div className="settings-section-head">
            <div>
              <h2>Negocio</h2>
              <p>Estos valores se usan para metas y para el cierre diario o semanal.</p>
            </div>
          </div>
          {cargando ? <div className="card muted">Cargando configuración…</div> : <>
            <div className="settings-grid">
              ...
              <div className="field"><label>Tipo de cierre</label><select value={tipoCierre} onChange={e=>setTipoCierre(e.target.value as CloseMode)}><option value="daily">Cierre diario</option><option value="weekly">Cierre semanal (lunes a domingo)</option></select></div>
