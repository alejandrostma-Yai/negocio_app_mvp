...
const initial:QuickData={name:"Usuario",workingCapital:0,houseFund:0,goalBalance:0,goalAmount:0,grossToday:0,dailyGoal:0,createdToday:0,sales:[],phones:[],notes:[],dailyClosed:false,weeklyClosed:false};
...
export default function VistaRapida(){
  ...
  const load=useCallback(async()=>{
    ...
    setData({
      ...
      grossToday:gross,dailyGoal:Number(settings.data?.daily_unit_goal??0),createdToday:created.count??0,sales:rows,phones:phoneRows,notes:(notes.data??[]) as Note[],dailyClosed:Boolean(dailyClose.data),weeklyClosed:Boolean(weeklyClose.data)
    });
    setLoading(false);
  },[]);
  ...
  return <AuthGuard><main className="quick-view-shell">
    ...
    <section className="quick-countdowns">
      <CloseCountdown mode="daily" closed={data.dailyClosed}/>
      <CloseCountdown mode="weekly" closed={data.weeklyClosed}/>
    </section>
