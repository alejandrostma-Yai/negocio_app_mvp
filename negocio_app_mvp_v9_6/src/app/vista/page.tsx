...
const initial:QuickData={name:"Usuario",workingCapital:0,houseFund:0,goalBalance:0,goalAmount:0,grossToday:0,monthSales:0,dailyHouseAmount:0,dailyGoal:0,createdToday:0,completedToday:0,totalClients:0,sales:[],phones:[],notes:[],dailyClosed:false,weeklyClosed:false,closeMode:"daily"};
...
export default function VistaRapida(){
  ...
  const load=useCallback(async()=>{
    ...
    const [wallet,settings,sales,completed,created,notes,dailyClose,weeklyClose,monthCompleted,totalClients]=await Promise.all([
      ...
      supabase.from("settings").select("goal_amount,daily_unit_goal,daily_house_amount,close_mode").eq("user_id",user.id).single(),
    ...
    setData({
      ...
      grossToday:gross,monthSales:monthGross,dailyHouseAmount:Number(settings.data?.daily_house_amount??0),dailyGoal:Number(settings.data?.daily_unit_goal??0),createdToday:created.count??0,completedToday:completedRows.length,totalClients:totalClients.count??0,
  ...
  return <AuthGuard><main className="v96-shell">
    ...
    <section className="v96-finance-grid">
      <article className="v96-card v96-house-card">
        ...
        <strong className="v96-big-money orange">{loading?"…":`RD$${money(data.houseFund)}`}</strong>
      ...
      <article className="v96-capital-card">
        ...
        <strong>{loading?"…":`RD$${money(data.workingCapital)}`}</strong>
      ...
      <article className="v96-card v96-goal-card">
        ...
        <strong className="v96-goal-amount">RD${money(data.goalAmount)}</strong>
        ...
        <p>Meta: RD${money(data.goalAmount)}<br/>Alcanzado: RD${money(data.goalBalance)}</p>
    ...
    <section className="v96-metrics">
      <Link href="/" className="v96-metric cyan"><Icon kind="chart"/><div><span>Monto bruto de hoy</span><strong>RD${money(data.grossToday)}</strong><small>Ver detalle ›</small></div></Link>
