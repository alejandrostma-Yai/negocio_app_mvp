...
export default function Home() {
  ...
  const capitalCierre = data.completedToday * 2705;
  const casaCierre = Math.min(3000, Math.max(data.grossToday - capitalCierre, 0));
  ...
  async function cerrarDia() {
    ...
    setMensaje("Día cerrado correctamente. El dinero fue distribuido.");
    setCerrando(false);
    await cargar();
  }
  ...
  return (
    <AuthGuard>
      <main className="shell">
        ...
        <h2 className="section-title">Cierre del día</h2>
        <div className="card close-day-card">
          {data.closedToday ? (
            <div className="close-day-done">Día cerrado ✓</div>
          ) : (
            <>
              <div className="muted small-text">Distribuye las ventas completadas de hoy con estas reglas: RD$2,705 por orden para capital y hasta RD$3,000 una sola vez para Casa. El resto va a la meta.</div>
              <button className="btn" type="button" onClick={() => setMostrarCierre(v => !v)} disabled={cargando}>
                {mostrarCierre ? "Ocultar resumen" : "Cerrar día"}
              </button>
              {mostrarCierre && (
                <div className="close-day-summary">
                  <div><span>Completadas hoy</span><strong>{data.completedToday}</strong></div>
                  <div><span>Bruto</span><strong>RD${money(data.grossToday)}</strong></div>
                  <div><span>Capital</span><strong>RD${money(capitalCierre)}</strong></div>
                  <div><span>Casa</span><strong>RD${money(casaCierre)}</strong></div>
                  <div><span>Meta</span><strong>RD${money(metaCierre)}</strong></div>
                  {agendaPendiente.length > 0 && <p className="close-day-warning">Hay {agendaPendiente.length} venta(s) pendiente(s) de hoy. Si tu configuración impide cerrar con pendientes, primero complétalas o cancélalas.</p>}
                  <button className="btn" type="button" onClick={cerrarDia} disabled={cerrando || data.completedToday === 0}>
