...
export default function VistaRapida(){
  ...
  // v10: cálculo potencial idéntico al cierre diario de Supabase:
  // bruto -> reposición de capital (cantidad de órdenes × MP1+MP2+MP3) -> Casa -> Meta.
  const capitalPerSale=data.mp1+data.mp2+data.mp3;
  const closeGoalAllocation=(gross:number,count:number)=>{
    const replacement=count*capitalPerSale;
    if(gross<replacement)return 0;
    const afterCapital=Math.max(gross-replacement,0);
    const house=Math.min(data.dailyHouseAmount,afterCapital);
    return Math.max(gross-replacement-house,0);
  };
  const currentGoalAllocation=closeGoalAllocation(data.grossToday,data.completedToday);
  ...
  const projectedGoalAllocation=closeGoalAllocation(projectedGross,projectedCompleted);
  const scheduledGoalIncrease=Math.max(projectedGoalAllocation-currentGoalAllocation,0);
  const scheduledGoalIncreasePct=data.goalAmount>0?scheduledGoalIncrease/data.goalAmount*100:0;
  ...
  return <AuthGuard><main className="v96-shell">
    ...
    <section className="v96-finance-grid">
      ...
      <article className="v96-card v96-goal-card">
        ...
        <div className="v10-goal-potential">
          <span>Si completas todo lo agendado hoy</span>
          <strong>{loading?"…":pendingSales.length>0?`+${scheduledGoalIncreasePct.toFixed(1)}%`:`+0.0%`}</strong>
          <small>{pendingSales.length>0?`Podrías llegar a ${projectedGoalProgress.toFixed(1)}%`:`No hay trabajo pendiente agendado hoy.`}</small>
        </div>
