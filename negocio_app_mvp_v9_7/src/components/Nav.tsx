...
export default function Nav() {
  ...
  async function salir() {
    ...
    if (user?.email) {
      ...
      try {
        localStorage.setItem("og-last-user", JSON.stringify({
          email: user.email,
          name: nombrePerfil || nombreCuenta
        }));
      } catch {}
    }
    ...
    <>
      <header className="topbar">
        ...
        <div className="topbar-actions">
          {pathname === "/" && (
            <Link className="topbar-focus" href="/vista" aria-label="Abrir vista rápida" title="Vista rápida">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <circle cx="12" cy="12" r="7"/><circle cx="12" cy="12" r="2.5"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2"/>
              </svg>
            </Link>
          )}
